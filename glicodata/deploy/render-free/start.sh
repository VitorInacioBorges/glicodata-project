...
if [[ -n "${GLICODATA_BOOTSTRAP_ADMIN_CODE:-}" || -n "${GLICODATA_BOOTSTRAP_ADMIN_PASSWORD:-}" ]]; then
    echo "Provisionando administrador inicial..."
    php artisan glicodata:admin-create --from-env
else
    echo "Bootstrap administrativo não configurado; seguindo sem criar administrador."
fi
